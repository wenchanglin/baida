...
- (instancetype)initWithFrame:(CGRect)frame SegmentViewHeight:(CGFloat)segmentViewHeight titleArray:(NSArray *)titleArray Controller:(UIViewController *)controller lineWidth:(float)lineW lineHeight:(float)lineH{
    ...
    if (self = [super initWithFrame:frame]){
        ...
        for (int i = 0; i < self.nameArray.count; i++) {
            ...
            [button setTitleColor:[UIColor colorWithHexString:@"#E0BE8D"] forState:UIControlStateNormal];
            [button setTitleColor:[UIColor colorWithHexString:@"#990000"]forState:UIControlStateSelected];
            [button addTarget:self action:@selector(Click:) forControlEvents:(UIControlEventTouchUpInside)];
            [self.segmentView addSubview:button];
            [_buttonArray addObject:button];   //添加顶部按钮
        }
        
        self.indicateView = [[UILabel alloc]initWithFrame:CGRectMake((avgWidth-lineW)/2,segmentViewHeight-lineH-10, lineW, lineH)];
        self.indicateView.backgroundColor = [UIColor colorWithHexString:@"#990000"];
