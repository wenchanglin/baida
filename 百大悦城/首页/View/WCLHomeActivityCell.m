...
{
    _models = models;
    [_sfbackImgeView sd_setImageWithURL:[NSURL URLWithString:models.activityPic] placeholderImage:[UIImage imageNamed:@"banner_placehoder"]];
    ...
    [RACObserve(models, acivityType)subscribeNext:^(id  _Nullable x) {
        ...
        else if ([x isEqualToString:@"SCORE"])
        {
            ...
            [self.applyBtn setTitle:[NSString stringWithFormat:@"%@积分报名",models.needScore.length>0?models.needScore:@"0"] forState:UIControlStateNormal];
            self.applyBtn.rac_command = [[RACCommand alloc]initWithSignalBlock:^RACSignal * _Nonnull(id  _Nullable input) {
        ...
        {
            [self.applyBtn setTitle:@"现金报名" forState:UIControlStateNormal];
            self.applyBtn.rac_command = [[RACCommand alloc]initWithSignalBlock:^RACSignal * _Nonnull(id  _Nullable input) {
